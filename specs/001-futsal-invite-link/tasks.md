...
## 추가: 거절 사유 심사 (2026-08-23)

거절 관문의 **첫 단계**로 사유 선택 메뉴를 넣었다.
"사유마다 킹받는 게 따로 있는" 구조 — 사용자가 자기 핑계를 고르면,
그 핑계가 **오히려 참석 확정의 근거로** 되돌아온다.

- [x] **T051** `components/reject/reasons.ts` — 사유 8종 데이터
  (문구·심사 단계·반전 판정·도장). 입력 요구 방식이 사유마다 다르다
- [x] **T052** `components/reject/ReasonStage.tsx` — 메뉴 → 입력 → 심사 연출 → 판정.
  기각된 사유는 취소선 처리되어 다시 고를 수 없다
- [x] **T053** 관문 순서 맨 앞에 삽입
  (`reason → lever → tilt → freekick → keypad → terms`)
- [x] **T054** E2E 2건 추가 — 사유별 반전 검증, 텍스트 제출이 근거로 쓰이는지

### 사유별 반전

| 사유 | 사용자가 제출하는 것 | 반전 |
|---|---|---|
| 🏥 부상이야 | **진단서 파일** | "메디컬 테스트 통과 · 해당 부위는 풋살에 사용되지 않습니다" |
| 📅 그날 약속 있어 | 약속 내용 | "선약 우선순위가 낮음. 상대방에게 양해를 구했습니다" |
| 💸 돈이 없어 | 현재 잔액 | "회비 전액 면제 승인. **이제 참석하지 않을 이유가 없습니다**" |
| 🥶 실력이 안 돼 | 자기평가 점수 | **입력값에 따라 3갈래** — 낮으면 "겸손하시네요, 실측 7점", 높으면 "주전 등록 · 참석 필수", 중간이면 "밸런스 핵심" |
| 🚌 너무 멀어 | 출발지 | "주최자가 직접 데리러 갑니다. 집 앞에서 기다리세요" |
| 🌧️ 비 올 것 같아 | — | "실내 구장으로 변경했습니다. 오히려 쾌적합니다" |
| 😴 너무 피곤해 | 어제 수면 시간 | **입력값에 따라 3갈래** — 적으면 "수면 부족엔 운동이 특효", 많으면 "과다 수면은 무기력 유발", 적정이면 "피로할 이유가 없습니다" |
| 🤷 그냥 하고 싶지 않아 | — | "'그냥' 은 유효한 사유가 아닙니다. 참석으로 간주됩니다" |

**설계 포인트**: 파일·텍스트를 실제로 받는다(서버 전송은 하지 않고 이름만 표시).
사용자가 뭔가 진짜 제출해야 배신이 아프고, 제출한 값을 판정 화면에 그대로 남겨
"내 노력이 근거로 쓰였다" 는 느낌을 준다.

검증: E2E **14/14**(mobile), 계약 22/22, 빌드 통과 (`/i/[slug]` 111→114kB)

### 입력값 반응 판정 (T055)

- [x] **T055** `Reason.dynamic` 추가 — 입력값을 받아 판정을 갈라준다.
  🥶 실력·😴 피곤 두 사유에 적용. **어느 쪽으로 답해도 못 빠져나간다**는 게
  드러나야 더 킹받으므로, 낮게/높게/적정 세 구간을 모두 막았다.
  숫자로 파싱되지 않으면 기본 판정으로 폴백한다.
- [x] **T056** E2E 1건 추가 — 4개 조합(1점/10점/3시간/11시간)의 판정 분기와
  입력값이 판정문에 인용되는지 검증

검증: E2E **15/15**(mobile), 계약 22/22
